...
async function main() {
  ...
  console.log(`  Upserted/updated ${r.upserted} admin_users with a mapped role.`);
  console.log(`  Skipped ${r.skippedNoRole} members with no mapped panel role.`);
  ...
  if (r.pruned > 0) {
    console.log(`  Pruned ${r.pruned} discord_sync admin row(s) no longer in sync set.`);
  }
  console.log("Done.");
}
