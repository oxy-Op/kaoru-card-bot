...
async function main() {
  console.log(
    `[Editions] Generating ${EDITIONS_PER_CHAR} editions per character (batch ${BATCH_SIZE}, start ID ${START_ID})`,
  );
  ...
  for (let i = 0; i < chars.length; i += BATCH_SIZE) {
        ...
        try {
          ...
          for (const type of types) {
            try {
              ...
              const baseWeight = computeRarityWeight(
                char.popularity ?? 0,
                char.role,
              );
    ...
    if ((i / BATCH_SIZE) % 10 === 0) {
      console.log(
        `[Editions] Progress: ${i}/${chars.length} chars, +${totalGenerated} editions, ${totalSkipped} skipped, ${totalFailed} failed`,
      );
    }
  }

  console.log(
    `[Editions] Done! Generated: ${totalGenerated}, Skipped: ${totalSkipped}, Failed: ${totalFailed}`,
  );
  process.exit(0);
}
